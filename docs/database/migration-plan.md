...
| 003 | `categories` | table, `UNIQUE(business_id, name_key)`, `UNIQUE(id, business_id)`, `type` CHECK, guard trigger **`categories_guard_default_immutable()`** | guard logic ships with the table so identity edits are impossible from day one |
| 004 | `transactions` | table, UNIQUE(id,business_id), CHECKs, **composite FK `(category_id,business_id) → categories(id,business_id) ON DELETE RESTRICT`** | core ledger event; category dependency enforced now |
...
| 009 | `seed_default_categories` | `seed_default_categories_for_business()` (`SECURITY DEFINER`, owned by `postgres`, `search_path` pinned) + AFTER INSERT trigger on `businesses` | defaults appear the moment a business exists; narrow scope, no generic permissions |
